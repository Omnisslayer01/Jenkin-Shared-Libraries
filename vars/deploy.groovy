def call(String container_name){
  ...
  sh "docker stop mysql_db django_cont nginx || true"
  sh "docker rm mysql_db django_cont nginx || true"
  
  sh "docker run -d -p '3306:3306' --name mysql_db --network=mynetwork -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=db mysql"
  sh "docker run -d -p '8000:8000' --name django_cont --network=mynetwork -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=db -e MYSQL_HOST=mysql_db -e MYSQL_USER='root' ${container_name}"
